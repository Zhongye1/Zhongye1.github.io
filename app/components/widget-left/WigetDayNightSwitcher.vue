<template>
  <!--
    外层是唯一的交互元素（role / 键盘 / 悬停都在这里），也是组件的根节点 ——
    这样调用方传下来的 aria-label、title 才会落到这颗开关上；
    内层 .button-box 只负责画，它上面的 clip-path 会把 outline 一起裁掉，
    所以键盘焦点环也画在外层（见样式里的 .switcher:focus-visible）。
  -->
  <span
    class="switcher"
    role="switch"
    :aria-checked="ballStatus"
    tabindex="0"
    @click="ballTrans"
    @keydown.enter.prevent="ballTrans"
    @keydown.space.prevent="ballTrans"
...
const props = defineProps({
  ...
  status: {
    type: Boolean,
    default: false,
    // 受控状态: 外部主题当前是否夜间。用于首屏对齐, 以及主题在别处被改动时同步球的位置
  },
})
...
// 球的初始位置由外部主题决定，避免「站点已经是夜间、按钮还停在白天」
const ballStatus = ref(props.status)

// 主题在别处被改动时（系统主题切换、其它入口）把球同步过去。
// 点击这颗球时 ballStatus 已经自己改过，判等后直接返回，不会重复触发动画；
// 悬停动画的计时器一并收掉，免得残留的星星 / 流星继续跑。
watch(
  () => props.status,
  (value) => {
    if (value === ballStatus.value) return
    ballStatus.value = value
    dayHoverAnimationReset()
    nightHoverAnimationReset()
  },
)

// 顺带把触发它的那次交互一起抛出去：调用方要靠点击坐标决定圆形揭示动画从哪开始
// （键盘触发时坐标为空，调用方自己退到焦点元素中心，见 composables/useTheme.ts）
const ballTrans = (event) => {
  if (ballStatus.value) {
    ballStatus.value = false
    emit('onStatus', ballStatus.value, event)
    dayHoverAnimation()
    nightHoverAnimationReset()
...
const ifTwinkle = (id) => {
  return twinkleList.some(
    (twinkleStar) => twinkleStar.twinkleShowStatus && id === twinkleStar.twinkleId,
  )
}
...
/*
    外层是交互元素（role / 键盘 / 悬停都在上面），也负责让组件能以行内元素的身份嵌进侧栏那一行。
    焦点环必须画在这里：.button-box 上的 clip-path 会把 outline 一起裁掉。
*/
.switcher {
  display: inline-flex;
  /* 侧栏那一行是 flex，别让窄屏把它挤扁（内层盒子的宽高是写死的） */
  flex: none;
  border-radius: 999px;
}
  ...
  .star-cloud-box {
    ...
    .star-box {
      ...
      .meteor {
        ...
        border-radius: 50%;
        transform: translate(calc(var(--box-height) * 1.75), calc(var(--box-height) * -0.35))
          rotate(255deg);
    ...
    .halo-linear {
      ...
      .halo-right {
        transform: translateX(
          calc(var(--box-width) - 50% - var(--ball-margin) - var(--ball-size) / 2)
        );
      }
    }
...
@keyframes meteor-fall {
  ...
  40% {
    transform: translate(calc(var(--box-height) * -0.625), calc(var(--box-height) * 0.3))
      rotate(255deg);
  }
