...
title: 2025-10-28 关于前端网络请求（XHR、Ajax、Fetch、Axios）
mathjax: true
abbrlink: 61262
...
status 是 XMLHttpRequest 对象的一个属性，用于表示 HTTP 状态码。
...
responseType 是 XMLHttpRequest 对象的属性，用于指定响应的数据类型。它决定了如何解析从服务器返回的响应数据。
...
相对于 Fetch、XMLHttpRequest 和 Ajax，还是更喜欢 Axios。主要是有简洁易用的 API，统一的错误处理和拦截器支持，取消请求和超时处理功能，以及基于 Promise 的链式调用和跨浏览器兼容， Axios 更方便、高效，并提供更好的开发体验