...
function showWelcome() {
  ...
  let dist = getDistance(113.380873,23.046287, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
  ...
  try {
    ...
    document.getElementById("welcome-info").innerHTML =
      `<b><center> GZHU.aic 已提交登录信息 </center>&emsp;&emsp;欢迎来自 <span style="color:var(--theme-color)">${pos}</span> 的同志，${timeChange}您现在距离站长约 <span style="color:var(--theme-color)">${dist}</span> 公里，已定位当前的IP地址为： <span style="color:var(--theme-color)">${ip}</span>， ${posdesc}</b>`;
  } catch (err) {
...
——————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
  ...
  那个......
  平常的日子里不要老是压抑
  也许一切都会改观的，我也许不会继续孤独下去吧
  无论如何也要将自己的生活继续下去
  也许这一切之后会拥有一个美好的未来，我是如此的祈祷着……
...
其中一张附在这里。https://pic4.zhimg.com/80/v2-1f36bcd074963f0e9c5df29e9ed4fa07_720w.webp
...
function createtime2() {
  ...
  setTimeout(
    console.log.bind(
      ...
      "",
      "",
      'background:url("https://www.talaxy.site/static/fb1ffc836d6d829688c0ba281ed33cca/3999e/img.png") no-repeat;font-size:450%'
    )
  );
  ...
  setTimeout(
    console.warn.bind(console, "%c S013-782 %c 已定位访问源,监控中", "color:white; background-color:#d9534f", "")
  );
}
createtime2();
...
function share_() {
  ...
  try {
    ...
    navigator.clipboard.writeText('柊野的博客分享\n标题：' + subTitle + '\n链接：' + url + '\n📋');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息📋",
          message: "您现在可以通过粘贴去分享了！",
          position: 'top-left',
          offset: 50,
          showClose: true,
          type: "success",
